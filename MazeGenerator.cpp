...
private:
    ...
    void generateClassicImpl() {
        ...
        startX = rand() % width;
        startY = rand() % height;
        do { finishX = rand() % width; finishY = rand() % height; }
    ...
    void draw(sf::RenderWindow& window) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                sf::RectangleShape wall(sf::Vector2f(cellSize, 6));
                wall.setFillColor(sf::Color(55,65,85));
...
public:
    ...
    void draw(sf::RenderWindow& win) {
        ...
        sf::RectangleShape wall(sf::Vector2f(cell, 5));
        wall.setFillColor(sf::Color(100,100,100,40));
        for (int y = 0; y < h; y++) for (int x = 0; x < w; x++) {
...
int main() {
    ...
    sf::Image icon;
    icon.loadFromFile("frames/frame_0001.png");
    menuWin.setIcon(icon.getSize().x, icon.getSize().y, icon.getPixelsPtr());
