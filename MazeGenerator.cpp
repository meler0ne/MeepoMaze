...
private:
    ...
    void loadFrames() {
        ...
        for (int i = 1; i <= 6; i++) {
            ...
            else { ss.str(""); ss << "double_frames/frame_000" << i << ".png";
            ...
            while (win.pollEvent(ev)) {
                ...
                if (ev.type == sf::Event::MouseMoved) {
                    ...
                    if (playBtn.getGlobalBounds().contains(m.x, m.y)) {
                        if (!hover) { hover = true; playBtn.setFillColor(sf::Color(100,170,230));
                            playBtn.setOutlineColor(sf::Color(180,220,255)); }
                    } else if (hover) { hover = false; playBtn.setFillColor(sf::Color(70,130,200));
