...
// Функция для конвертации русских строк в формат SFML
...
// ==================== КЛАСС ДЛЯ КРАСИВОЙ ОТРИСОВКИ ПУТИ ====================
class BeautifulPathRibbon {
private:
    sf::VertexArray ribbon;      // Основная лента
    sf::VertexArray glow;         // Свечение вокруг ленты
    sf::VertexArray pattern;      // Декоративный узор
    float thickness;              // Толщина ленты
    float progress;               // Прогресс анимации (0 до 1)
    std::vector<std::pair<int, int>> pathCells; // Клетки пути
...
public:
    ...
    void updateRibbon() {
        ...
        // Собираем точки для видимой части пути
        ...
        // Добавляем интерполированную точку для плавности
        ...
        // Создаем ленту из треугольных полос
...
private:
    ...
    // Получение точки на пути по прогрессу
...
struct Cell {
    bool wallTop = true;      // Стена сверху
    bool wallRight = true;    // Стена справа
    bool wallBottom = true;   // Стена снизу
    bool wallLeft = true;     // Стена слева
    bool visited = false;     // Посещена ли при генерации
    bool path = false;        // Входит ли в найденный путь
    int distance = 0;         // Расстояние от старта (для BFS)
    int parentX = -1;         // Координата X родителя для восстановления пути
    int parentY = -1;         // Координата Y родителя для восстановления пути

    float animProgress = 0.0f;    // Прогресс анимации появления
    float targetProgress = 0.0f;   // Целевой прогресс
};

// ==================== ОСНОВНОЙ КЛАСС ЛАБИРИНТА ====================
class Maze {
private:
    int width, height;           // Размеры в клетках
    int cellSize;                // Размер клетки в пикселях
    std::vector<std::vector<Cell>> grid;  // Сетка лабиринта

    std::string currentGenAlgorithm = "Рекурсивный"; // Используемый алгоритм
    int pathLength = 0;           // Длина найденного пути
    float solveTime = 0.0f;       // Время поиска пути

    bool isSolving = false;       // Идет ли анимация решения
    bool pathCompleted = false;   // Завершена ли анимация пути
    const float ANIMATION_SPEED = 0.001f; // Скорость анимации

    std::vector<std::pair<int, int>> pathCells; // Клетки найденного пути
    float globalLineProgress = 0.0f; // Прогресс отрисовки пути (0-1)

    BeautifulPathRibbon pathRibbon; // Красивая лента пути
    ...
    int startX, startY;           // Позиция старта
    int finishX, finishY;         // Позиция финиша

    // Проверка, слишком ли близко финиш к старту
    ...
    // Рекурсивный алгоритм генерации лабиринта (бэктрекинг)
    void carvePath(int x, int y) {
        grid[y][x].visited = true;

        // Направления: вверх, вправо, вниз, влево
        int directions[4][3] = {
            {0, -1, 0}, // dx, dy, тип стены
            {1, 0, 1},
            {0, 1, 2},
            {-1, 0, 3}
        };
    ...
    void findPathBFS() {
        ...
        while (!q.empty() && !found) {
            ...
            // Проверяем все 4 направления, если нет стены
            // Вверх
        ...
        // Если путь найден, восстанавливаем его от финиша к старту
    ...
    // Получение позиции головы змейки (для анимации движения)
...
public:
    ...
    Maze(int w, int h, int cell = 40) : width(w), height(h), cellSize(cell), pathRibbon(cell) {
        ...
        if (!font.loadFromFile("C:/Windows/Fonts/arial.ttf")) {
            // Если не удалось загрузить, пробуем локальный файл
            if (!font.loadFromFile("arial.ttf")) {
                // Если и это не удалось, выводим сообщение (но программа продолжит работу)
                std::cout << "Предупреждение: Не удалось загрузить шрифт" << std::endl;
            }
        }
        ...
        // Загрузка текстуры для Meepo с проверкой
        ...
        // Загрузка текстуры для финиша с проверкой
    ...
    // Генерация рекурсивным алгоритмом
    ...
    // Генерация пустого лабиринта (только внешние стены)
    void generateEmpty() {
        reset();
        std::srand(static_cast<unsigned int>(std::time(NULL)));
        ...
        // Все стены уже true по умолчанию, ничего не меняем
    }

    // Генерация открытого лабиринта (без внутренних стен)
    void generateFull() {
        reset();
        std::srand(static_cast<unsigned int>(std::time(NULL)));
    ...
    // Обработка кликов мыши для редактирования стен
    void handleMouseClick(int mouseX, int mouseY, bool leftButton, bool rightButton) {
        if (isSolving) return; // Не редактируем во время анимации
        ...
        if (cellX >= 0 && cellX < width && cellY >= 0 && cellY < height) {
            ...
            // Определяем, по какой стене кликнули (ближе к краю)
            bool nearTop = relY < 5;
            ...
            if (leftButton) {
                ...
                if (nearTop) {
                    ...
                    return; // Кликнули не по стене
                }
            }
            else if (rightButton) {
                // Правая кнопка - убрать стену
                if (nearTop) {
            ...
            clearPath(); // Очищаем путь, так как стены изменились
        }
    }

    // Очистка найденного пути
    void clearPath() {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                grid[y][x].path = false;
    ...
    bool saveToFile(const std::string& filename) {
        try {
            ...
            // Сохраняем позиции старта и финиша
            file << startX << " " << startY << std::endl;
            file << finishX << " " << finishY << std::endl;

            // Сохраняем состояние всех стен
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    file << grid[y][x].wallTop << " "
                        << grid[y][x].wallRight << " "
                        << grid[y][x].wallBottom << " "
                        << grid[y][x].wallLeft << std::endl;
                }
            }

            file.close();
            std::cout << "Лабиринт сохранен в файл: " << filename << std::endl;
            return true;
        }
    ...
    bool loadFromFile(const std::string& filename) {
        try {
            ...
            if (!file.is_open()) {
                std::cerr << "Ошибка: Не удалось открыть файл для загрузки: " << filename << std::endl;
                return false;
            }
            ...
            if (w != width || h != height) {
                std::cerr << "Ошибка: Размеры лабиринта в файле не совпадают!" << std::endl;
                file.close();
                return false;
            }

            // Загружаем позиции старта и финиша
            file >> startX >> startY;
            file >> finishX >> finishY;

            // Загружаем состояние стен
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    file >> grid[y][x].wallTop
                        >> grid[y][x].wallRight
            ...
            file.close();
            clearPath(); // Очищаем найденный путь
            std::cout << "Лабиринт загружен из файла: " << filename << std::endl;
            return true;
        }
        catch (const std::exception& e) {
    ...
    void draw(sf::RenderWindow& window) {
        ...
        // Рисуем красивую ленту пути
        if (!pathCells.empty() && globalLineProgress > 0.001f) {
            pathRibbon.draw(window);
        }

        // Рисуем стартовую клетку
        ...
        // Рисуем Meepo на старте или в пути
...
// ==================== КЛАСС ФОНОВОГО ЛАБИРИНТА ДЛЯ МЕНЮ ====================
...
private:
    ...
    const float REGENERATION_TIME = 7.0f; // Время смены лабиринта
...
public:
    ...
    // Обновление меню, возвращает true если нажата кнопка
...
int main()
{
    // Установка кодировки для консоли (для русского языка)
    ...
    std::string saveFileName = "maze_save.txt"; // Имя файла для сохранения
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            if (event.type == sf::Event::KeyPressed) {
                ...
                case sf::Keyboard::S:
                    ...
                    if (maze.saveToFile(saveFileName)) {
                        // Визуальный отклик можно добавить позже
                    }
                    break;
                case sf::Keyboard::L:
                    // Загрузка лабиринта
                    if (maze.loadFromFile(saveFileName)) {
                        // Визуальный отклик можно добавить позже
                    }
                    break;
        ...
        std::string info =
            ...
            "[C] Очистить путь\n"
            "[S] Сохранить лабиринт\n"
            "[L] Загрузить лабиринт\n"
            "[ESC] Выход\n\n"
            "МЫШЬ:\n"
            "Левая кнопка - переключить стену\n"
