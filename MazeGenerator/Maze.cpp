...
private:
    ...
    void findPathBFS(int startX, int startY, int targetX, int targetY) {
...
public:
    ...
    void solveBFS() {
        ...
        findPathBFS(0, 0, width - 1, height - 1);
    ...
    void draw(sf::RenderWindow& window) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                if (cell.path && !(x == 0 && y == 0) && !(x == width - 1 && y == height - 1)) {
                    ...
                    pathRect.setFillColor(sf::Color(173, 216, 230, alpha));
        ...
        startRect.setFillColor(sf::Color(176, 196, 222, 255));
        startRect.setPosition(3.0f, 3.0f);
        window.draw(startRect);

        if (textureLoaded && !isSolving && !pathCompleted) {
            float startX = cellSize / 2.0f;
            float startY = cellSize / 2.0f;

            snakeSprite.setPosition(startX, startY);
        ...
        if (finishLoaded) {
            float finishX = static_cast<float>((width - 1) * cellSize + cellSize / 2);
            float finishY = static_cast<float>((height - 1) * cellSize + cellSize / 2);
            finishSprite.setPosition(finishX, finishY);
...
public:
    StartMenu() : backgroundMaze(15, 10, 45) {
        ...
        // Верхние точки градиента (светлее)
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseMoved) {
                ...
                if (buttonBounds.contains(static_cast<float>(mousePos.x), static_cast<float>(mousePos.y))) {
                    buttonHovered = true;
                    // При наведении - более яркие цвета
    ...
    void draw(sf::RenderWindow& window) {
        ...
        // Рисуем тень под кнопкой
        window.draw(buttonShadow);

        // Рисуем градиент кнопки
